...
def plot_seq2seq_preds(model, data_dict, state, pred_steps, split='test',
                       scaler=None, ax=None, state2idx=None,
                       return_preds_only=False, return_truth_only=False):
    if ax is None:
        ax = plt.gca()
    ...
    if return_truth_only:
        return X_te_tail, tpre, y_true, tpost
